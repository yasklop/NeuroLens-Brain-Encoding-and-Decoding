...
from dc_ldm.sd15_for_fmri_clipA import fSD15  # Import SD1.5 adapter
...
def normalize(img):
    ...
    img = torch.tensor(img, dtype=torch.float32)  # Explicitly use float32
...
def fmri_transform(x, sparse_rate=0.2):
    ...
    x_aug[idx] = 0
    return torch.tensor(x_aug, dtype=torch.float32)  # Explicitly use float32
...
def main(config):
    ...
    # create generative model - choose between original LDM and SD1.5
    use_sd15 = getattr(config, 'use_sd15', True)  # Default to SD1.5, can be overridden in config
    
    if use_sd15:
        print("Using Stable Diffusion 1.5 adapter...")
        generative_model = fSD15(pretrain_mbm_metafile, num_voxels,
                    device=device, logger=config.logger, 
                    ddim_steps=config.ddim_steps, global_pool=config.global_pool, use_time_cond=config.use_time_cond,
                    cfg_scale=config.cfg_scale)
    else:
        print("Using original LDM...")
    ...
    if config.checkpoint_path is not None:
        ...
        if use_sd15:
            # For SD1.5 adapter, load state dict directly
            generative_model.load_state_dict(model_meta['model_state_dict'])
        else:
            # For original LDM, load into model attribute

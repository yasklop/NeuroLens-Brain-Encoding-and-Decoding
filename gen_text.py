...
def get_img_label(class_index:dict, img_filename:list, naive_label_set=None):
    ...
    for _, file in enumerate(img_filename):
        ...
        for c, (w, d) in enumerate(zip(wind, desc)):
            if name in w:
                img_label.append((c, d, nl, prompt))
                break
...
class Kamitani_dataset(Dataset):
    def __init__(self, fmri, image, img_label, fmri_transform=identity, image_transform=identity, num_voxels=0, num_per_sub=50):
        ...
        self.prompt = [i[3] for i in img_label]
    ...
    def __getitem__(self, index):
        ...
        return {'fmri': self.fmri_transform(fmri), 'image': self.image_transform(img),
                'image_class_name': img_class_name, 'prompt': prompt}
...
def main():
    train, test =create_Kamitani_dataset(subjects=['sbj_3'])

    import csv
    rows = []
    with open('data/Kamitani/npz/imagenet_testing_label.csv', 'r', newline='', encoding='utf-8') as f:
        csvreader = csv.reader(f)
    ...
    for i in range(len(test)):
        file_name = test[i]['file_name']

        img_array = np.clip(test[i]['image'], 0, 1)
        ...
        inputs = processor(text=prompt, images=pil_img, return_tensors="pt").to(device, torch_dtype)
        ...
        parsed_answer = processor.post_process_generation(generated_text, task="<MORE_DETAILED_CAPTION>", image_size=(pil_img.width, pil_img.height))

   
        for j, row in enumerate(rows):
            if len(row) > 1 and row[1] == file_name:  
                row.append(parsed_answer)
                print(f"file_name: {file_name} found")
                break
            else:
                print(f"file_name: {file_name} not found")
                

    with open('data/Kamitani/npz/imagenet_testing_label.csv', 'w', newline='', encoding='utf-8') as f:
